...
import static com.github.mccullocht.nvector.NativeHnswVectorsFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
import static com.github.mccullocht.nvector.NativeHnswVectorsReader.SIMILARITY_FUNCTIONS;
 ...
 * @lucene.experimental
 */
public final class NativeHnswVectorsWriter extends KnnVectorsWriter {

    private static final long SHALLOW_RAM_BYTES_USED = RamUsageEstimator
            .shallowSizeOfInstance(NativeHnswVectorsWriter.class);
    ...
    public NativeHnswVectorsWriter(
        ...
        String metaFileName = IndexFileNames.segmentFileName(
                state.segmentInfo.name, state.segmentSuffix, NativeHnswVectorsFormat.META_EXTENSION);
        ...
        try {
            ...
            CodecUtil.writeIndexHeader(
                    meta,
                    NativeHnswVectorsFormat.META_CODEC_NAME,
                    NativeHnswVectorsFormat.VERSION_CURRENT,
    ...
    public KnnFieldVectorsWriter<?> addField(FieldInfo fieldInfo) throws IOException {
        FieldWriter<?> newField = FieldWriter.create(
     ...
     * @param levelNodeOffsets where to place the new offsets for the nodes in the
     *                         vector index.
     * @return The graph
    ...
    private void reconstructAndWriteNeighbours(
        ...
        // Now that we have sorted, do delta encoding to minimize the required bits to
        // store the
        // information
    ...
    public void mergeOneField(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
        CloseableRandomVectorScorerSupplier scorerSupplier = flatVectorWriter.mergeOneFieldToIndex(fieldInfo,
                mergeState);
        boolean success = false;
        try {
            long vectorIndexOffset = vectorIndex.getFilePointer();
            // build the graph using the temporary vector data
            // we use NativeHnswVectorsReader.DenseOffHeapVectorValues for the graph
            // construction
            ...
            if (scorerSupplier.totalVectorCount() > 0) {
                ...
                switch (fieldInfo.getVectorEncoding()) {
                    case BYTE ->
                        mergedVectorValues = KnnVectorsWriter.MergedVectorValues.mergeByteVectorValues(fieldInfo,
                                mergeState);
                    case FLOAT32 ->
                        mergedVectorValues = KnnVectorsWriter.MergedVectorValues.mergeFloatVectorValues(fieldInfo,
                                mergeState);
                }
                graph = merger.merge(
                        mergedVectorValues,
     ...
     * @return The non-cumulative offsets for the nodes. Should be used to create
     *         cumulative offsets.
     * @throws IOException if writing to vectorIndex fails
     */
