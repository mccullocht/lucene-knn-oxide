...
/**
 * Lucene 9.9 vector format, which encodes numeric vector values into an
 * associated graph connecting
 * the documents having values. The graph is used to power HNSW search. The
 * format consists of two
 * files, and requires a {@link FlatVectorsFormat} to store the actual vectors:
 *
 * <h2>.vex (vector index)</h2>
 *
 * <p>
 * Stores graphs connecting the documents for each field organized as a list of
 * nodes' neighbours
 * as following:
 *
 * <ul>
 ...
 * <li><b>[vint]</b> the maximum number of connections (neighbours) that each
 * node can have
 ...
 * <li><b>array[vint]</b> for levels greater than 0 list of nodes on this level,
 * stored as
 ...
 * @lucene.experimental
 */
public final class NativeHnswVectorsFormat extends KnnVectorsFormat {

    static final String META_CODEC_NAME = "NativeHnswVectorsFormatMeta";
    static final String VECTOR_INDEX_CODEC_NAME = "NativeHnswVectorsFormatIndex";
    static final String META_EXTENSION = "vemn";
    static final String VECTOR_INDEX_EXTENSION = "vexn";
     ...
     *
     * <p>
     * NOTE: We eagerly populate `float[MAX_CONN*2]` and `int[MAX_CONN*2]`, so
     * exceptionally large
     ...
     * construction. This
     * maximum value preserves the ratio of the
     * `DEFAULT_BEAM_WIDTH`/`DEFAULT_MAX_CONN` (i.e. `6.25 *
     * 16 = 3200`).
     */
    public static final int MAXIMUM_BEAM_WIDTH = 3200;

    /**
     * Default number of the size of the queue maintained while searching during a
     * graph construction.
     */
     ...
     * node. Defaults to
     * {@link NativeHnswVectorsFormat#DEFAULT_MAX_CONN}. See {@link HnswGraph} for
     * more details.
     */
    private final int maxConn;

    /**
     * The number of candidate neighbors to track while searching the graph for each
     * newly inserted
     * node. Defaults to {@link NativeHnswVectorsFormat#DEFAULT_BEAM_WIDTH}. See
     * {@link HnswGraph}
     * for details.
     */
    ...
    private static final NativeFlatVectorsFormat flatVectorsFormat = new NativeFlatVectorsFormat(
    ...
    public NativeHnswVectorsFormat() {
     ...
     */
    public NativeHnswVectorsFormat(int maxConn, int beamWidth) {
        this(maxConn, beamWidth, DEFAULT_NUM_MERGE_WORKER, null);
    }

    /**
     * Constructs a format using the given graph construction parameters and scalar
     * quantization.
     *
     * @param maxConn         the maximum number of connections to a node in the
     *                        HNSW graph
     ...
     *                        doing merge. If
     *                        larger than 1, a non-null {@link ExecutorService} must
     *                        be passed as mergeExec
     * @param mergeExec       the {@link ExecutorService} that will be used by ALL
     *                        vector writers that are
     *                        generated by this format to do the merge. If null, the
     *                        configured {@link
     *                        MergeScheduler#getIntraMergeExecutor(MergePolicy.OneMerge)}
     *                        is used.
     */
    public NativeHnswVectorsFormat(
            int maxConn, int beamWidth, int numMergeWorkers, ExecutorService mergeExec) {
        super("NativeHnswVectorsFormat");
    ...
    public KnnVectorsWriter fieldsWriter(SegmentWriteState state) throws IOException {
        return new NativeHnswVectorsWriter(
                state,
                maxConn,
                beamWidth,
    ...
    public KnnVectorsReader fieldsReader(SegmentReadState state) throws IOException {
        return new NativeHnswVectorsReader(state, flatVectorsFormat.fieldsReader(state));
    }
    ...
    public String toString() {
        return "NativeHnswVectorsFormat(name=NativeHnswVectorsFormat, maxConn="
                + maxConn
                + ", beamWidth="
                + beamWidth
                + ", flatVectorFormat="
